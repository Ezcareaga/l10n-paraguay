...
class L10nPyAccountMigrationWizard(models.TransientModel):
    ...
    def action_apply(self):
        ...
        if self.mode == "clean":
            if not self.confirm_destructive:
                raise UserError(
                    _("Modo 'limpia' requiere confirmar la operación destructiva.")
                )
            self._apply_clean()
    ...
    def _apply_clean(self):
        ...
        _logger.info(
            "Chart 'py' cargado en modo clean para company %s", self.company_id.name
        )

    def _apply_assisted(self):
        # No destructivo: solo loguea y deja el chart como está; manda activity
        self.company_id.partner_id.message_post(
            body=_(
